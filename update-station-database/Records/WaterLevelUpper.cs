...
namespace Krafta.Records
{
	...
	public struct WaterLevelUpper : ISqlRecord
	{
		...
		{
			string baseInsertCommand = "INSERT INTO water_level_upper (date, water_level, state) " +
			                           "VALUES(@date, @water_level, @state)";

			string baseSelectCommand = "SELECT date FROM water_level_upper WHERE date LIKE @date";
