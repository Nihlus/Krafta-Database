...
{
	/// <summary>
	/// Represents an input water level record for the upper station.
	/// </summary>
	public struct WaterLevelUpper : ISqlRecord
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="Krafta.Records.WaterLevelUpper"/> struct.
		/// Takes a raw line from "OVY.SKV" as input.
		/// </summary>
		/// <param name="InRecord">In record.</param>
		public WaterLevelUpper(string InRecord)
		{
		...
		{
			return "CREATE TABLE water_level_upper (" +
		...
		{
			string baseInsertCommand = "INSERT INTO water_level_upper (date, time, water_level, state) " +
			                           "VALUES(@date, @time, @water_level, @state)";

			string baseSelectCommand = "SELECT date, time FROM water_level_upper WHERE date LIKE @date AND time LIKE @time";
