...
{
	/// <summary>
	/// Represents an input temperature record for the G3 turbine.
	/// </summary>
	public struct TempG3 : ISqlRecord
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="Krafta.Records.TempG3"/> struct.
		/// Takes a raw line from "TEMPG3.SKV" as input.
		/// </summary>
		/// <param name="InRecord">In record.</param>
		public TempG3(string InRecord)
		{
			...
			{
				this.TurbineBearingTemperature = double.Parse(Utilities.Math.CorrectNumericRecordValue(recordParts[4], 1), NumberStyles.Any, CultureInfo.InvariantCulture);
			}

			if (recordParts.Length > 5)
			{
				this.State = recordParts[5] != "OFF";
			}
			else
			{
				this.State = true;
