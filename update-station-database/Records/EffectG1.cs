...
{
	/// <summary>
	/// Represents an input effect record for the G1 turbine.
	/// </summary>
	public struct EffectG1 : ISqlRecord
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="Krafta.Records.EffectG1"/> struct.
		/// Takes a raw line from "EFFEKT.SKV" as input.
		/// </summary>
		/// <param name="InRecord">In record.</param>
		public EffectG1(string InRecord)
		{
		...
		{
			string baseInsertCommand = "INSERT INTO effect_G1 (date, time, effect, state) VALUES(@date, @time, @effect, @state)";
			string baseSelectCommand = "SELECT date, time FROM effect_G1 WHERE date LIKE @date AND time LIKE @time";
