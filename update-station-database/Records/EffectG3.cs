...
namespace Krafta.Records
{
	...
	public struct EffectG3 : ISqlRecord
	{
		...
		public EffectG3(string InRecord)
		{
			...
			{
				this.Effect = uint.Parse(recordParts[3]);
			}
		...
		{
			string baseInsertCommand = "INSERT INTO effect_G3 (date, time, effect, state) VALUES(@date, @time, @effect, @state)";
			string baseSelectCommand = "SELECT date, time FROM effect_G3 WHERE date LIKE @date AND time LIKE @time";
