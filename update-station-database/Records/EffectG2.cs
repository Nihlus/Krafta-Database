...
{
	/// <summary>
	/// Represents an input effect record for the G2 turbine.
	/// </summary>
	public struct EffectG2 : ISqlRecord
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="Krafta.Records.EffectG2"/> struct.
		/// Takes a raw line from "EFFEKT.SKV" as input.
		/// </summary>
		/// <param name="InRecord">In record.</param>
		public EffectG2(string InRecord)
		{
		...
		{
			string baseInsertCommand = "INSERT INTO effect_G2 (date, time, effect, state) VALUES(@date, @time, @effect, @state)";
			string baseSelectCommand = "SELECT date, time FROM effect_G2 WHERE date LIKE @date AND time LIKE @time";
