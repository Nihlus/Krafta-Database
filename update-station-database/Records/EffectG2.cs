...
namespace Krafta.Records
{
	...
	public struct EffectG2 : ISqlRecord
	{
		...
		{
			string baseInsertCommand = "INSERT INTO effect_G2 (date, effect, state) VALUES(@date, @effect, @state)";
			string baseSelectCommand = "SELECT date FROM effect_G2 WHERE date LIKE @date";
