...
namespace Krafta.Records
{
	public struct FlywheelG2 : ISqlRecord
	{
		...
		{
			string baseInsertCommand = "INSERT INTO flywheel_G2 (date, throttle, hatch_level, OVY, state) VALUES(@date, @throttle, @hatch_level, @OVY, @state)";
			string baseSelectCommand = "SELECT date FROM flywheel_G2 WHERE date LIKE @date";
